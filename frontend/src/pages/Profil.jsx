...
const UserProfile = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await APIUser.post('/user/profile', { firstName, lastName, address, phone }, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setSuccess('Profil mis à jour avec succès');
            setError('');
            setEditMode(false); // retour en lecture seule
        } catch (err) {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-gray-100 p-4">
            <div className="bg-white p-8 rounded-2xl shadow-md w-full max-w-md">
                ...
                {!firstName && !lastName && !address && !phone && !editMode ? (
                    <p className="text-center text-gray-500 mb-4">Aucune information de profil disponible.</p>
                ) : null}
